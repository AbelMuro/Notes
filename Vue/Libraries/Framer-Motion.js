...
<template>
    <motion.div 
          ...
          :custom="index" />            //custom prop is used to pass a value to the dynamic variant callback
</template>
...
<script setup>
    ...
    const createSequenceAnimation = async () => {
          await animate(ref.value, { x: 0 }, {duration: 0.5})             // you can target an element directly with re.value           
          await animate(ref.value, { x: 100}, {duration: 0.5, delay: 2})                   
          await animate(ref.value, { x: 20}, {duration: 0.5})
          await animate('.someClass', {y: 45}, {duration: 0.4})         // you can target an element using css selectors within a specified scope
    }
...
<template>
        <motion.div ref='ref'></div>                                    'we target an element directly'
    
        <div ref='ref'>                                                 'we create a scope that can be used to select elements using css selectors'
            <div class='someClass'/>
        </div>
</template>
  ...
  const isOnDropZone = (e, info) => { 
       ...
       if(info.point.x >= dropZone.left && info.point.x <= dropZone.right && 
          info.point.y >= dropZone.top && info.point.y <= dropZone.bottom)
              droppedItems.value = [...droppedItems.value, 'new item'] 
  }
...
<template>
    <div>
        ...
        <motion.div id="drop-zone">
              Drop Here
              <div v-for="(item, index) in droppedItems" :key="item"/>
        </motion.div>
    </div>
</template>
...
<script setup>
    ...
    const containerRef = useTemplateRef('my-div');

</script>


<template>
      <div ref="my-div" :style="{overflow: 'hidden'}"> //(this must have a scroll bar)
          <motion.div 
            :initial="{opacity: 0}"
            :whileInView="{opacity: 1}"   
            :viewport="{
                once: true,           // animation will only occur once
                root: 'my-div',   // by assigning a ref object here, the container that has this same ref will act as the viewport
...
<script setup>
    ...
    const ref = useTemplateRef('my-div');
...
<template>
      <motion.div class='grid' layout>       
           <motion.div class='box' layout/>          // setting layout to the child components can fix a distortion bug that occurs sometimes
           <motion.div class='box' layout/>  
           <motion.div class='box' layout/>
      </motion.div>   
</template>
...
<template>
      <motion.div class='gridWithScroll' layoutScroll :style="{overflow: 'scroll'}">     
         <motion.div class='box' layout></motion.div>    
         <motion.div class='box' layout></motion.div>    
         <motion.div class='box' layout></motion.div>
      </motion.div>    
</template>
...
<script setup>
  ...
  const openFirstAccordion = ref(false);
  const openSecondAccordion = ref(false);

  const handleOpenFirstAccordion = () => {
      openFirstAccordion.value = true;
  }

  const handleOpenSecondAccordion = () => {
      openSecondAccordion.value = true;
  }
...
<template>
  <LayoutGroup>
          <motion.div layout @click="handleOpenFirstAccordion">               //this component will expand when we click on it, triggering a smooth animation
              <motion.h2 layout> 'header' </motion.h2>
              <motion.div v-if="openFirstAccordion" layout> 
                   'Content'   
               </motion.div>
          </motion.div>
          <motion.div layout @click="handleOpenSecondAccordion">               //this component will expand when we click on it, triggering a smooth animation
              <motion.h2 layout> 'header' </motion.h2>
              <motion.div v-if="openSecondAccordion" layout> 
                   'Content'   
               </motion.div>
          </motion.div>
  </LayoutGroup>
